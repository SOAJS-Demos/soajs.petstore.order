...
var lib = {
	...
	"sendMail": function (apiName, req, data, cb) {
		...
		if(!(servicesConfig.orders && servicesConfig.orders.mail && servicesConfig.orders.mail[apiName])){
			return cb("Error in mail configuration");
		}
		var mailOptions = {
			'to': data.email,
			'from': servicesConfig.mail.from,
			'subject': servicesConfig.orders.mail[apiName].subject,
			'data': data
		};
		if (servicesConfig.orders.mail[apiName].content) {
			mailOptions.content = servicesConfig.orders.mail[apiName].content;
		} else if (servicesConfig.orders.mail[apiName].path) {
			mailOptions.path = servicesConfig.orders.mail[apiName].path;
		}
